...
const TotalOrders = () => {
  ...
  const fetchTotalOrders = async (start = "", end = "") => {
    setLoading(true);
    try {
      // Build query parameters if dates are provided
      let url = "/api/admin/orders/total";
  ...
  const updateDeliveryStatus = async (orderId, status) => {
    try {
      ...
      if (res.data.status === 200) {
        fetchTotalOrders(startDate, endDate); // Refresh with current filters
      }
    } catch (err) {
  ...
  const handleDateFilter = () => {
    fetchTotalOrders(startDate, endDate);
  };
  ...
  return (
    <div className="total-orders-container">
      <div className="orders-header">
        <h3 className="total-orders-title">All Orders</h3>
