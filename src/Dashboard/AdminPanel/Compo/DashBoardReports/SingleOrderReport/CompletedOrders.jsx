...
const CompletedOrders = () => {
  ...
  const fetchCompletedOrders = async (start = "", end = "") => {
    setLoading(true);
    try {
      // Build query parameters if dates are provided
      let url = "/api/admin/orders/completed";
  ...
  const updateDeliveryStatus = async (orderId, status) => {
    try {
      ...
      if (res.data.status === 200) {
        fetchCompletedOrders(startDate, endDate); // Refresh with current filters
      }
    } catch (err) {
  ...
  return (
    <div className="completed-orders-container">
      <div className="orders-header">
        <h3 className="completed-orders-title">Completed Orders</h3>
      ...
      {loading ? (
        ...
        <div className="no-orders-container">
          {startDate || endDate ? "No completed orders found for the selected date range" : "No completed orders found"}
        </div>
      ) : (
