...
const PendingOrders = () => {
  ...
  const fetchPendingOrders = async (start = "", end = "") => {
    setLoading(true);
    try {
      // Build query parameters if dates are provided
      let url = "/api/admin/orders/pending";
  ...
  const updateDeliveryStatus = async (orderId, status) => {
    try {
      ...
      if (res.data.status === 200) {
        fetchPendingOrders(startDate, endDate); // Refresh with current filters
      }
    } catch (err) {
  ...
  const handleDateFilter = () => {
    fetchPendingOrders(startDate, endDate);
  };
    ...
    <div className="pending-orders-container">
      ...
      {loading ? (
        ...
        <div className="no-orders-container">
          {startDate || endDate ? "No pending orders found for the selected date range" : "No pending orders found"}
        </div>
      ) : (
